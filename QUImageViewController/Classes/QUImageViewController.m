...
#import "QUImageViewController.h"

#import "QUSimpleImageDownloader.h"
#import "UIImage+QUImageEffects.h"
#import "UIApplication+QUImageViewController.h"

CG_INLINE CGFLOAT_TYPE QUImageFloatAbs(CGFLOAT_TYPE aFloat) {
...
// Public Constants
CGFloat const QUImageViewController_DefaultAlphaForBackgroundDimmingOverlay = 0.66f;
CGFloat const QUImageViewController_DefaultBackgroundBlurRadius = 2.0f;

// Private Constants
static CGFloat const QUImageViewController_MinimumBackgroundScaling = 0.94f;
static CGFloat const QUImageViewController_TargetZoomForDoubleTap = 3.0f;
static CGFloat const QUImageViewController_MaxScalingForExpandingOffscreenStyleTransition = 1.25f;
static CGFloat const QUImageViewController_TransitionAnimationDuration = 0.3f;
static CGFloat const QUImageViewController_MinimumFlickDismissalVelocity = 800.0f;
...
} QUImageViewControllerStartingInfo;
...
@interface QUImageViewController ()
<
...
// General Info
@property (strong, nonatomic, readwrite) QUImageInfo *imageInfo;
@property (strong, nonatomic, readwrite) UIImage *image;
@property (assign, nonatomic, readwrite) QUImageViewControllerTransition transition;
@property (assign, nonatomic, readwrite) QUImageViewControllerMode mode;
@property (assign, nonatomic, readwrite) QUImageViewControllerBackgroundOptions backgroundOptions;
@property (assign, nonatomic) QUImageViewControllerStartingInfo startingInfo;
@property (assign, nonatomic) QUImageViewControllerFlags flags;
...
@implementation QUImageViewController
...
- (instancetype)initWithImageInfo:(QUImageInfo *)imageInfo
    ...
    if (self) {
        ...
        if (_mode == QUImageViewControllerMode_Image) {
...
- (void)showFromViewController:(UIViewController *)viewController
    ...
    if (self.mode == QUImageViewControllerMode_Image) {
        if (transition == QUImageViewControllerTransition_FromOffscreen) {
    ...
    } else if (self.mode == QUImageViewControllerMode_AltText) {
...
- (void)dismiss:(BOOL)animated {
    ...
    if (self.mode == QUImageViewControllerMode_AltText) {
        [self dismissByExpandingAltTextToOffscreenPosition];
    }
    else if (self.mode == QUImageViewControllerMode_Image) {
        
        if (_flags.imageIsFlickingAwayForDismissal) {
            [self dismissByCleaningUpAfterImageWasFlickedOffscreen];
        }
        else if (self.transition == QUImageViewControllerTransition_FromOffscreen) {
            ...
            BOOL useCollapsingThumbnailStyle = (startingRectForThumbnailIsNonZero
                                                && self.image != nil
                                                && self.transition != QUImageViewControllerTransition_FromOffscreen);
...
- (UIInterfaceOrientationMask)supportedInterfaceOrientations {
    ...
    /*
     ...
     QUImageViewController while the **device** is oriented in
     ...
     the rotation corrections during presentation. - August 31, 2014 QU.
     */
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    ...
    [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext> context) {
        ...
        QUImageViewControllerFlags flags = strongSelf.flags;
        flags.isRotating = NO;
...
- (void)deviceOrientationDidChange:(NSNotification *)notification {
    ...
    /*
     ...
     This makes it difficult to preserve the desired behavior of QUImageViewController. 
...
- (void)setupImageAndDownloadIfNecessary:(QUImageInfo *)imageInfo {
    if (imageInfo.image) {
        ...
        typeof(self) __weak weakSelf = self;
        NSURLSessionDataTask *task = [QUSimpleImageDownloader downloadImageForURL:imageInfo.imageURL canonicalURL:imageInfo.canonicalImageURL completion:^(UIImage *image) {
            typeof(self) strongSelf = weakSelf;
...
- (void)showImageViewerByExpandingFromOriginalPositionFromViewController:(UIViewController *)viewController {
    ...
    if (self.backgroundOptions & QUImageViewControllerBackgroundOption_Blurred) {
    ...
    [viewController presentViewController:self animated:NO completion:^{
        ...
        CGFloat duration = QUImageViewController_TransitionAnimationDuration;
        if (USE_DEBUG_SLOW_ANIMATIONS == 1) {
            duration *= 4;
        }
        
        __weak QUImageViewController *weakSelf = self;
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [UIView
                 ...
                 animations:^{
                     ...
                     if ([UIApplication sharedApplication].QU_usesViewControllerBasedStatusBarAppearance) {
                     ...
                     CGFloat scaling;
                     if (!(weakSelf.backgroundOptions & QUImageViewControllerBackgroundOption_Scaled)) {
                         scaling = 1.0;
                     } else {
                         scaling = QUImageViewController_MinimumBackgroundScaling;
                     }
                     ...
                     if (weakSelf.backgroundOptions & QUImageViewControllerBackgroundOption_Blurred) {
...
- (void)showImageViewerByScalingDownFromOffscreenPositionWithViewController:(UIViewController *)viewController {
    ...
    [viewController presentViewController:self animated:NO completion:^{
        ...
        [self updateScrollViewAndImageViewForCurrentMetrics];
        CGFloat scaling = QUImageViewController_MaxScalingForExpandingOffscreenStyleTransition;
            ...
            [UIView
             ...
             animations:^{
                 ...
                 if (!(weakSelf.backgroundOptions & QUImageViewControllerBackgroundOption_Scaled)) {
                     ...
                     targetScaling = QUImageViewController_MinimumBackgroundScaling;
                 }
...
- (CGFloat)alphaForBackgroundDimmingOverlay {
    ...
    if ([self.optionsDelegate respondsToSelector:@selector(alphaForBackgroundDimmingOverlayInImageViewer:)]) {
        ...
        alpha = QUImageViewController_DefaultAlphaForBackgroundDimmingOverlay;
    }
...
- (CGFloat)backgroundBlurRadius {
    ...
    if ([self.optionsDelegate respondsToSelector:@selector(backgroundBlurRadiusForImageViewer:)]) {
        ...
        blurRadius = QUImageViewController_DefaultBackgroundBlurRadius;
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseInOut animations:^{
                ...
                if ([UIApplication sharedApplication].QU_usesViewControllerBasedStatusBarAppearance == NO) {
                    [[UIApplication sharedApplication] setStatusBarHidden:_startingInfo.statusBarHiddenPriorToPresentation
...
- (void)dismissByExpandingAltTextToOffscreenPosition {
    ...
    [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        CGFloat targetScale = QUImageViewController_MaxScalingForExpandingOffscreenStyleTransition;
...
- (UIView *)blurredSnapshotFromParentmostViewController:(UIViewController *)viewController {
    ...
    // the performance of QU_applyBlurWithRadius:tintColor:saturationDeltaFactor:maskImage:
    ...
    UIImage *blurredImage = [image QU_applyBlurWithRadius:blurRadius tintColor:nil saturationDeltaFactor:1.0f maskImage:nil];
...
- (void)updateLayoutsForCurrentOrientation {
    ...
    if (_flags.rotationTransformIsDirty) {
        ...
        if (_flags.isPresented) {
            ...
            CGFloat targetScaling;
            if (!(self.backgroundOptions & QUImageViewControllerBackgroundOption_Scaled)) {
                targetScaling = 1.0;
            } else {
                targetScaling = QUImageViewController_MinimumBackgroundScaling;
            }
...
- (void)updateScrollViewAndImageViewForCurrentMetrics {
    ...
    BOOL usingOriginalPositionTransition = (self.transition == QUImageViewControllerTransition_FromOriginalPosition);
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if (scrollView.zoomScale == 1 && (QUImageFloatAbs(velocity.x) > 1600 || QUImageFloatAbs(velocity.y) > 1600 ) ) {
        [self dismiss:YES];
    }
}
...
- (void)imageDoubleTapped:(UITapGestureRecognizer *)sender {
    ...
    if (self.scrollView.zoomScale == 1.0f) {
        ...
        CGFloat zoomWidth = self.view.bounds.size.width / QUImageViewController_TargetZoomForDoubleTap;
        CGFloat zoomHeight = self.view.bounds.size.height / QUImageViewController_TargetZoomForDoubleTap;
        targetZoomRect = CGRectMake(point.x - (zoomWidth/2.0f), point.y - (zoomHeight/2.0f), zoomWidth, zoomHeight);
        targetInsets = [self contentInsetForScrollView:QUImageViewController_TargetZoomForDoubleTap];
    } else {
        self.scrollView.accessibilityHint = self.accessibilityHintZoomedOut;
...
- (void)dismissingPanGestureRecognizerPanned:(UIPanGestureRecognizer *)panner {
    ...
    if (panner.state == UIGestureRecognizerStateBegan) {
        ...
        if (vectorDistance > QUImageViewController_MinimumFlickDismissalVelocity) {
            if (_flags.isDraggingImage) {
